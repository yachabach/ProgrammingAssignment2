...
## makeCacheMatrix creates a cache location for a matrix and it's inverse.
## It also defines functions to access both.  We do this 
## in the environment where these functions are defined using the <<- operator.
## This is normally the Global Environment.  Doing so ensures we don't lose the
## value (and the computer effort) when we leave a function.  As
## long as the user is in an environment where makeCacheMatrix is available, the 
...
makeCacheMatrix <- function(x = matrix()) {
        ...
        ## 
        ## This function uses the <<- operator to
        ## put the passed matrix (y) into the parent frame memory location (x).  
        ...
        ## matrix that was passed to the function.  It uses the <<- operator to
        ## store the result in the parant environment
        ## 
        ...
        ## and therefore the cache, with vec$setMatrix, vec$getMatrix, 
        ## vec$setInverse, vec$getInverse.
        ## 
